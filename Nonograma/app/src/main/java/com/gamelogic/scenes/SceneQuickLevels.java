...
//
        ...
        levels.add(createLevel(engine,"4x4", posX, posY, sizeX, sizeY, 4, 4, 10));

        posX = logicWidth / 2 - sizeX / 2;
        levels.add(createLevel(engine,"5x5", posX, posY, sizeX, sizeY, 5, 5, 12));

        posX = (int) (logicWidth / 1.38 - sizeX / 2);
        levels.add(createLevel(engine,"5x10", posX, posY, sizeX, sizeY, 5, 10, 15));

        posY = logicHeight / 2 - sizeY / 2;
        posX = (int) (logicWidth / 3.5 - sizeX / 2);
        levels.add(createLevel(engine,"8x8", posX, posY, sizeX, sizeY, 8, 8, 15));

        posX = logicWidth / 2 - sizeX / 2;
        levels.add(createLevel(engine,"10x10", posX, posY, sizeX, sizeY, 10, 10, 20));

        posX = (int) (logicWidth / 1.38 - sizeX / 2);
        levels.add(createLevel(engine,"10x15", posX, posY, sizeX, sizeY, 10, 15, 25));
    ...
    private Button createLevel(Engine engine, String text, int x, int y, int sizeX, int sizeY, final int i, final int j, int reward) {
        final Button button = new Button(text, x, y, sizeX, sizeY) {
            ...
            public void update(double deltaTime) {
                if (fade.getFadeOutComplete() && isSelected()) {
                    setSelected(false);
                    engine.getGame().pushScene(new SceneGame(i, j, reward));
                    fade.reset();
